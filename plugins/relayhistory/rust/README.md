...
Cloud onboarding flow. It owns the upload coordinator and RelayHistory transport
and links the local evidence engine directly; it does not load Node or invoke the
JSON helper. Its delivery cycle is a bounded
`relayhistory_plugin::delivery::worker::drain` over the same `RelayHistoryReceiver`, so
the background probe and foreground helper use the same probe-owned worker. The existing `relayhistory-plugin` remains Cargo's default executable.
