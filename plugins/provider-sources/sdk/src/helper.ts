/** Bounded, cancellable bridge to the optional provider-source Rust executable. */
...
export async function helperRequest<T>(operation: string, args: Record<string, unknown> = {}, options: HelperOptions = {}): Promise<T> {
  ...
  if (Buffer.byteLength(body) > 16 * 1_048_576) throw new InvalidArgumentError('Provider source helper request exceeds 16 MiB', 'INVALID_ARGUMENT');
  const executable = options.binaryPath ?? process.env.HISTORY_PROVIDER_SOURCES_BIN
    ?? packagedBinary();
  return new Promise<T>((resolve, reject) => {
    ...
    const abort = () => finish(new RelayHistoryError('Provider source helper operation cancelled', 'HISTORY_PLUGIN_CANCELLED'));
    const timer = setTimeout(() => finish(new RelayHistoryError('Provider source helper operation timed out', 'HISTORY_PLUGIN_TIMEOUT')), options.timeoutMs ?? 60_000);
    options.signal?.addEventListener('abort', abort, { once: true });
    child.on('error', () => finish(new RelayHistoryError('Provider source helper is unavailable; install its matching platform artifact', 'HISTORY_PLUGIN_BINARY_MISSING')));
    child.stdin.on('error', () => finish(new RelayHistoryError('Provider source helper input failed', 'HISTORY_PLUGIN_PROTOCOL_FAILED')));
    // Never forward arbitrary stderr, which could contain auth or history.
    child.stderr.resume();
    child.stdout.on('data', (chunk: Buffer) => {
      bytes += chunk.length;
      if (bytes > 32 * 1_048_576) return finish(new RelayHistoryError('Provider source helper response exceeds 32 MiB', 'HISTORY_PLUGIN_PROTOCOL_FAILED'));
    ...
    child.on('close', code => {
      ...
      try {
        ...
        if (!envelope.ok) {
          ...
          finish(new ErrorType(`Provider source operation failed (${code})`, code));
        } else finish(undefined, envelope.value);
      } catch { finish(new RelayHistoryError('Provider source helper returned an invalid response', 'HISTORY_PLUGIN_PROTOCOL_FAILED')); }
    });
