...
fn main() {
    ...
    std::thread::spawn(|| {
        ...
        eprintln!("Provider source helper deadline exceeded");
        std::process::exit(124);
    });
    if run().is_err() {
        eprintln!("Provider source helper protocol failure");
        std::process::exit(1);
    }
}
fn run() -> anyhow::Result<()> {
    ...
    let output = serde_json::to_vec(&history_provider_sources::helper::handle(request))?;
    anyhow::ensure!(output.len() <= MAX_OUTPUT, "response too large");
