...
/**
 * Point every optional helper entry of each plugin lockfile at the coordinates
 * its manifest declares. Exported so the release version script applies one
 * version through the same logic instead of repeating it.
 */
export async function syncHelperLocks(root = repositoryRoot) {
  for (const plugin of Object.keys(plugins)) {
    const directory = resolve(root, "plugins", plugin, "sdk");
    const manifest = JSON.parse(
      await readFile(resolve(directory, "package.json"), "utf8"),
    );
    const path = resolve(directory, "package-lock.json");
    const lock = JSON.parse(await readFile(path, "utf8"));
    validatePluginManifest(plugin, manifest);
    for (const [name, version] of Object.entries(
      manifest.optionalDependencies ?? {},
    )) {
      const platform = Object.keys(platforms).find((platform) =>
        name.endsWith("-" + platform),
      );
      if (!platform) throw new Error("Unexpected helper platform");
