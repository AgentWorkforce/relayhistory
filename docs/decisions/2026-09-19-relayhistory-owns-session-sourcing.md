...
along in a `session_meta` marker rather than as typed usage.

**Devin's control rows.** A `role: "system"` message node becomes a `system`
marker, a `user` node flagged `is_user_input: false` becomes a
`synthetic_turn` marker, and node `metadata.summarized_from` becomes a
`compaction_boundary` marker — which is why the row reads `◐` rather than `✓`:
the classified lifecycle records are preserved, but the store's remaining
control surface (`app_state`, `shell_last_seen_index`, `prompt_history`,
`cogs_json`) is deliberately not read.
