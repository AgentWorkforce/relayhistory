...
**Event rows, model, tool calls, file edits — cursor, grok, opencode, relay.**
`ingest_cursor_line` calls `insert_cursor_prompt` and nothing else.
...
full-evidence connector". These four sources are prompts-only. Model is `◐` for
