...
fn insert_session_event_with_provenance(
    ...
    conn.execute(
        "INSERT INTO session_events \
         (source, session_id, project, project_key, project_key_method, cwd, git_branch, message_id, parent_id, ts_ms, role, kind, text, model, token_json, provider, stop_reason, event_uid) \
         VALUES (?1, ?2, ?3, \
           COALESCE((SELECT s.project_key FROM sessions s WHERE s.source = ?1 AND s.session_id = ?2), ?17), \
         ...
         project_key=COALESCE((SELECT s.project_key FROM sessions s WHERE s.source = ?1 AND s.session_id = ?2), session_events.project_key, ?17), \
