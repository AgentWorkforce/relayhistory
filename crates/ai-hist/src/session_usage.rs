...
use crate::usage::{normalize_usage_str, NormalizedUsage, UsageAccounting, UsageCoverage};
use anyhow::Result;
...
/// Build the `session_requests` view for the column set this database has.
///
/// One row per model request. Only assistant rows with a message id
/// participate: a user turn is not a request, and a row with no message id
/// cannot be placed in one without guessing.
///
/// The grouping key is the provider's `request_id` when `session_events`
/// carries one and the message id otherwise. Claude writes one JSONL record
/// per content block — same `requestId`, same `message.id`, *different*
/// record `uuid` — and the store's `message_id` column holds that record
/// uuid. So on a database with the request-id column a multi-block turn is
/// one request, and without it each block reads as its own. The view is
/// rebuilt whenever that column appears, so the upgrade needs no second
/// change here.
///
/// `usage_variants` counts the *distinct* non-null `token_json` blobs in a
/// group. Claude copies the same blob onto every block, so the expected value
/// is 1; anything higher means the copies disagree and the request's usage is
/// not established. `MIN(e.id)` is a stable unique tiebreak for the keyset
/// order without a synthetic row id.
pub(crate) fn session_requests_view_ddl(has_request_id: bool) -> String {
    let (key, key_source) = if has_request_id {
        (
            "COALESCE(NULLIF(e.request_id, ''), e.message_id)",
            "CASE WHEN NULLIF(e.request_id, '') IS NOT NULL THEN 'request-id' ELSE 'message-id' END",
        )
    } else {
        ("e.message_id", "'message-id'")
    };
    format!(
        "CREATE VIEW session_requests AS
SELECT
    e.source AS source,
    e.session_id AS session_id,
    {key} AS request_key,
    {key_source} AS request_key_source,
...
WHERE e.role = 'assistant'
  ...
  AND e.message_id <> ''
GROUP BY e.source, e.session_id, {key}"
    )
}

/// Whether `session_events` carries the provider request id.
///
/// Read from the database rather than assumed, so one binary serves a store
/// that has been migrated and one that has not.
fn session_events_has_request_id(conn: &Connection) -> Result<bool> {
    Ok(conn
        .prepare("SELECT 1 FROM pragma_table_info('session_events') WHERE name = 'request_id'")?
        .exists([])?)
}

/// Whether the stored view matches the column set it is derived from.
///
/// A view whose grouping key predates the request-id column would keep
/// splitting one Claude turn into a request per content block — and would do
/// it silently, reporting a well-formed total several times too large. So a
/// stale view counts as outstanding migration work.
pub(crate) fn session_requests_view_is_current(conn: &Connection) -> Result<bool> {
    ...
    Ok(stored == session_requests_view_ddl(session_events_has_request_id(conn)?))
}

/// Create the view, replacing one built for a different column set.
pub(crate) fn ensure_session_requests_view(conn: &Connection) -> Result<()> {
    conn.execute_batch("DROP VIEW IF EXISTS session_requests")?;
    conn.execute_batch(&session_requests_view_ddl(session_events_has_request_id(
        conn,
    )?))?;
    Ok(())
}
...
pub enum RequestKeySource {
    ...
    RequestId,
    /// The message id, because the store has no request id for this record.
    MessageId,
}
...
pub struct SessionUsageSummary {
    pub source: String,
    pub session_id: String,
    /// Requests that contributed usage to these totals.
    ...
    pub first_ts_ms: i64,
    pub last_ts_ms: i64,
    /// Requests whose usage could not be counted, and why.
    pub diagnostics: Vec<UsageDiagnostic>,
    /// True when the totals overflowed `u64` and are therefore not reported
    /// as a number anyone should use. Never saturates silently.
    pub overflowed: bool,
}
        ...
        .map(|raw| {
            ...
            SessionRequest {
                ...
                request_key_source: RequestKeySource::parse(&raw.request_key_source)
                    .unwrap_or(RequestKeySource::MessageId),
                source: raw.source,
...
pub fn session_usage_summary(
    ...
    let mut saw_any_evidence = false;
    ...
    let total = total.expect("evidence implies a running total");
    Ok(Some(SessionUsageSummary {
        ...
        provider_total_tokens: total.provider_total_tokens,
...
mod tests {
    ...
    fn one_claude_message_is_one_request_however_many_blocks_it_has() {
        ...
        assert_eq!(request.request_key_source, RequestKeySource::MessageId);
    ...
    fn disagreeing_usage_copies_are_refused_with_a_diagnostic() {
        ...
        assert_eq!(session_usage_summary(&conn, "claude", "s1").unwrap(), None);
    }
    ...
    #[test]
    fn a_session_with_no_usage_evidence_has_no_summary() {
        let conn = db();
        event(
            &conn,
            "claude",
