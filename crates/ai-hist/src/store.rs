...
/// `provider` and `stop_reason` join them from the OpenCode parser: the
/// upstream inference provider as the harness named it, and why the turn
/// ended as the harness reported it. Like the `sessions` list above, this
/// single declaration is both the migration list and the read-only guard
/// list, so the two cannot drift.
const REQUIRED_SESSION_EVENT_COLUMNS: &[&str] = &[
    "project_key",
    "project_key_method",
    "provider",
    "stop_reason",
...
const SESSION_EVENT_COLUMNS: &str =
    "id, source, session_id, project, project_key, cwd, git_branch, message_id, parent_id, \
     ts_ms, role, kind, text, model, token_json, provider, stop_reason, event_uid";
...
mod tests {
    ...
    ///
    /// Ported verbatim from #190 (head `4af44a5`), which hit this with the
    /// columns it adds; only the omitted column differs, because the two
    /// branches add different ones. Keeping the wording identical is what
    /// makes the eventual merge one hunk rather than two designs.
    #[cfg(feature = "delivery")]
    #[test]
    fn a_capture_trigger_with_an_outdated_payload_is_not_current() {
        ...
        let omitted = "'stop_reason',NEW.\"stop_reason\"";
